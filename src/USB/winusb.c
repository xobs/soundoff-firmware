...
static const struct winusb_simple_descriptor_set winusb_desc_set = {
    ...
    .dfu = {
        .function = {
            ...
            .wSubsetLength = (sizeof(struct winusb_function_subset_header) + sizeof(struct winusb_20_compatible_id_feature_descriptor) + sizeof(struct winusb_20_device_interface_guids_descriptor)),
        },
        .wcid = {.wLength = 20, .wDescriptorType = WINUSB_DT_MS_OS_20_FEATURE_COMPATIBLE_ID, .compatibleId = "WINUSB", .subCompatibleId = ""},
        .guids = {.wLength = sizeof(struct winusb_20_device_interface_guids_descriptor), .wDescriptorType = WINUSB_DT_MS_OS_20_FEATURE_REG_PROPERTY, .wPropertyDataType = WINUSB_PROP_DATA_TYPE_REG_MULTI_SZ, .wPropertyNameLength = 42, .PropertyName = u"DeviceInterfaceGUIDs", .wPropertyDataLength = 80, .PropertyData = u"{DA103DEF-26A6-47D9-970E-B3D561A277FA}\0"},
    },
#endif
};
